...
void greedySubsetSumWithLimit(const vector<vector<int>>& matrix, int targetSum, int q) {
    vector<pair<int, int>> elements; // Wektor przechowujacy pary (wartosc elementu, indeks wektora)

    // Przygotowanie pary (wartosc elementu, indeks wektora) dla kazdego elementu w wektorze wektorow
    ...
    // Sortowanie elementow malejaco wedlug wartosci
    sort(elements.rbegin(), elements.rend());

    int currentSum = 0;
    vector<int> subset;
    vector<int> vectorCount; // Licznik uzytych wektorow

    int iterator=0;
    while(currentSum <= targetSum && iterator < elements.size()) {
            if(currentSum + elements[iterator].first > targetSum)
            {
                iterator++;
                continue;
            }
...
int main() {
    for(int i=0; i<2; i++)
    {
        for(int no=1; no<=20; no++)
        {
            ...
            timeStart = clock();
            greedySubsetSumWithLimit(matrix, targetSum, q);
            timeEnd = clock();
